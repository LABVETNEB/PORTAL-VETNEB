...
$journal = Get-Content -Raw -Path $journalPath | ConvertFrom-Json
...
if (-not $existing) {
  ...
  $journal | ConvertTo-Json -Depth 10 | Set-Content -Path $journalPath -Encoding UTF8
  Write-Host "OK  drizzle\migrations\meta\_journal.json"
} else {
  Write-Host "SKIP drizzle\migrations\meta\_journal.json (entrada 0015 ya existe)"
}
...
if (-not $reportsJson.reports -or $reportsJson.reports.Count -eq 0) {
  throw 'No hay reportes para probar PR5'
}
...
Write-Host ''
Write-Host "Reporte seleccionado: $reportId (estado actual: $($report.currentStatus))"

if ($report.currentStatus -ne 'ready' -and $report.currentStatus -ne 'delivered') {
  Write-Host ''
  ...
  $statusResponse = Invoke-WebRequest `
    ...
    -Body (@{
      status = 'ready'
      note = 'Smoke PR5 report_access_tokens'
    } | ConvertTo-Json -Compress)
  ...
  if (-not $statusJson.success) {
    throw 'No se pudo dejar el reporte en estado ready para la prueba'
  }

  $report = $statusJson.report
}
...
$beforeAccessCount = $getTokenBeforeJson.reportAccessToken.accessCount
...
$afterAccessCount = $getTokenAfterJson.reportAccessToken.accessCount
if ($afterAccessCount -le $beforeAccessCount) {
  throw 'accessCount no incrementó luego del acceso público'
}
...
try {
  ...
  throw 'Error: el acceso público siguió funcionando después de la revocación'
}
catch {
...
'@
[System.IO.File]::WriteAllText($smokePath, $smoke, (New-Object System.Text.UTF8Encoding($false)))
Write-Host "OK  smoke-pr5-report-access-tokens.ps1"

Write-Host ''
Write-Host 'Fix runtime PR5 aplicado en C:\PORTAL-VETNEB'
