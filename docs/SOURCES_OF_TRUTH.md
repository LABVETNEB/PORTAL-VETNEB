...
| Evidence or approval reference | Árbol del repositorio y configuración GitHub verificados para `PR-AUDIT-ENTERPRISE-DOCS`; PR #1591 y closeout sanitizado de `PR-SEC-REPO-SETTINGS`; PR #1593 y closeout del bloque 03; PR #1601, canarias #1602/#1603 y PR correctiva #1605 del bloque 04; required checks efectivos, hardening de GitHub Actions y canarias #1616/#1618 del bloque 05 verificados el 2026-07-30; PR #1626 / Plan B slot 12 incorpora gobernanza de datos, recovery, incidentes y observabilidad el 2026-07-31; PR #1635 incorpora la fuente vigente de dashboard/rediseño Drive-like el 2026-08-04; E2E-GLOBAL-10B reconcilia las entradas CI/E2E con los workflows, el catálogo y la configuración GitHub releída en `c25f4e06` el 2026-09-16 ([acta](./implementation/e2e-global-10b-documentation-closeout.md)); el cierre post-programa #1749…#1757 recalcula el recenso E2E y marca `LIMPIEZA E2E` como `CLOSED` sobre `main@226588e39e30579a7f7935c7eeba59ae8d204403` el 2026-09-21 |
...
| CI / PR checks runbook | `docs/ops/CI_PR_CHECKS_RUNBOOK.md` | `docs/qa/regression-strategy.md`, `docs/governance/pr-readiness-review-checklist.md`, `docs/audit/enterprise-repository-maturity-audit-roadmap.md` | Vigente; reconciliado el 2026-09-16 (E2E-GLOBAL-10B) | Usar para checks de PR, merge y limpieza local, y como mapa operativo vigente de los workflows E2E, sus runners y los conteos de cohortes con su HEAD. Debe alinearse con los required checks reales; ante divergencia prevalecen la configuración efectiva de GitHub, los workflows y el catálogo |
| CI required checks and Actions policy | `docs/audit/pr-ci-required-checks-audit.md` | `docs/ops/CI_PR_CHECKS_RUNBOOK.md`, `docs/governance/enterprise-control-register.md`, `.github/workflows/backend-ci.yml`, `.github/workflows/frontend-ci.yml` | Closeout activo; bloque 05 `CLOSED` | Evidencia durable de los cuatro required checks con app ID, del hardening de GitHub Actions y de las canarias #1616/#1618; #1617 queda registrada como hallazgo diagnóstico, no como evidencia negativa. La configuración se releyó sin cambios el 2026-09-16 (cuatro contextos, `strict: true`, política `selected` con SHA pinning). No reemplaza los workflows ni la configuración efectiva de GitHub como fuentes ejecutables |
| CI always-run gates | `docs/audit/pr-ci-always-run-gates-audit.md` | `docs/architecture/ci-always-run-gates-rfc.md`, `docs/ops/CI_PR_CHECKS_RUNBOOK.md`, `.github/workflows/backend-ci.yml`, `.github/workflows/frontend-ci.yml`, `docs/governance/enterprise-control-register.md` | Closeout activo; bloque 04 `CLOSED` | Evidencia durable de #1601, canarias #1602/#1603 y corrección de rango de #1605; distingue contextos siempre presentes de heavies condicionales y fija el rango merge-base → head. Que no declare required los checks funcionales es el estado del bloque 04: desde el bloque 05, `validate-backend` y `validate-frontend` son required |
| CI / E2E completeness | `docs/ops/CI_PR_CHECKS_RUNBOOK.md` (secciones *Catálogo y cohortes E2E*, *E2E Completeness* y *Visual Regression Manual*) | `.github/workflows/e2e-completeness.yml`, `.github/workflows/frontend-ci.yml`, `.github/workflows/visual-regression-manual.yml`, `frontend/e2e/suites/catalog.ts`, `frontend/playwright.config.ts`, `docs/audit/LIMPIEZA E2E.md`; evidencia histórica: `docs/audit/pr-e2e-ci-completeness-audit.md`, `docs/audit/pr-e2e-ci-completeness-rfc.md` | Vigente operativo; slot 06 `CLOSED` como evidencia histórica | Estado recalculado en `226588e3` (2026-09-21): `e2e:ci` corre 69 specs / 1.041 tests con `next build` + `next start` en `Frontend CI` (required vía `validate-frontend`, heavy condicional por impacto), y `e2e:full` corre 100 specs / 1.337 tests con `next build` + `next start` en `E2E Completeness` (no required, en todo PR a `main` sin filtro de paths, más dispatch y schedule semanal). Ambas rutas usan el production runner (`VETNEB_E2E_PRODUCTION_RUNNER=1`): ninguna corre bajo `next dev`. Cifra histórica previa, no vigente: `c25f4e06` (2026-09-16) medía `ci` 67 / 1.030 y `full` 98 / 1.326. Las fuentes ejecutables son los workflows y el catálogo. Evidencia histórica del slot 06, fechada el 2026-07-30 y **no** vigente como inventario ni como configuración: PR #1620, head `2d9eda213d2a913786d2497ae18f345011d5eec7` y run `30567587561` / job `90955867044` probaron 72 specs / 786 tests (782 directos y 4 tras retry) con `e2e:full` bajo `next dev` y filtro de paths, ambos retirados después por E2E-GLOBAL-05B y E2E-GLOBAL-06 |
| Programa de saneamiento E2E (`LIMPIEZA E2E`) | `docs/audit/LIMPIEZA E2E.md` | `docs/implementation/e2e-global-01-completeness-apt-deb822.md` … `docs/implementation/e2e-global-10b-documentation-closeout.md`, `docs/ops/CI_PR_CHECKS_RUNBOOK.md`, `frontend/e2e/suites/catalog.ts` | `CLOSED` desde el 2026-09-21; se conserva como evidencia y closeout | Auditoría y programa **concluidos**: fases GLOBAL-01…11 ejecutadas y residuales cerrados o aceptados como DEFER con owner por #1749…#1757 (matriz final en su Anexo B, §B.7). Sus cifras de §§1–28 y del Anexo A son del baseline `2683f39a` (95 specs / 1.322 tests) y son históricas, no el estado actual. **El estado operativo vigente se consulta en el runbook, el catálogo y los workflows, no en este documento; no se reconstruye backlog nuevo desde él** |
...
| CI / E2E layering histórico | `docs/audit/e2e-ci-layering-strategy-audit.md` | `frontend/package.json`, `docs/ops/CI_PR_CHECKS_RUNBOOK.md`, `docs/qa/regression-strategy.md` | Vigente histórico parcial | Estrategia previa a PR-CI-3. El estado operativo de las cohortes y de la completitud E2E se lee en `docs/ops/CI_PR_CHECKS_RUNBOOK.md`, el catálogo y los workflows ejecutables; el closeout del slot 06 es sólo evidencia histórica |
| Auditorías E2E / suite de tests reemplazadas | `docs/HISTORICAL_DOCUMENTATION.md` (sección *Auditorías E2E superseded*) | `docs/audit/e2e-enterprise-organization-audit.md`, `docs/audit/test-suite-enterprise-architecture-audit.md` | `SUPERSEDED` desde el 2026-09-16 | Ambos documentos se conservan sólo por trazabilidad y no se usan como inventario ni como instrucción vigente: sus cifras (72/70 specs, cohortes por listas manuales) son de su fecha |
...
demostraron el 2026-07-30 la ruta automática de 72/72 specs con 786 tests, 782 pases
directos y 4 tras retry acotado. Esas cifras son evidencia histórica de ese closeout; el
inventario y el runner vigentes se leen en `docs/ops/CI_PR_CHECKS_RUNBOOK.md`. Los slots 08 y 09 agregan
...
Desde el 2026-09-16 (E2E-GLOBAL-10B) también separa el estado E2E operativo (runbook, workflows y
catálogo) de la evidencia histórica del slot 06 y marca como `SUPERSEDED` dos auditorías E2E
previas. Desde el 2026-09-21, `LIMPIEZA E2E` queda `CLOSED`: el programa de saneamiento E2E está
concluido y su documento se conserva como evidencia y closeout, mientras el estado E2E vigente se
lee en `docs/ops/CI_PR_CHECKS_RUNBOOK.md`, `frontend/e2e/suites/catalog.ts` y los workflows
ejecutables.
