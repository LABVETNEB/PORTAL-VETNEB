...
- `test/architecture/security/security-cross-tenant-idor-contract.test.ts` is a documentation-contract test covering 15 cross-tenant IDOR scenarios (`CTIDOR-001`..`CTIDOR-015`). Every entry hardcodes `productionReadinessStatus: "pending_runtime_staging_evidence"`; the test validates contract shape only, not actual runtime behavior.
