...
| P0-015 | Seguridad | P0 | Pendiente | Smoke cross-tenant / IDOR minimo + contrato `test/architecture/security/security-cross-tenant-idor-contract.test.ts` + matrices `docs/security/RBAC_MATRIX.md`, `docs/security/ENDPOINT_PERMISSION_MATRIX.md`, `docs/security/ENDPOINT_TEST_MATRIX.md` | `pnpm test` (guardrails) + revision de matrices + pruebas manuales con sesiones separadas en staging/produccion | Sin acceso a recursos de otro tenant; evidencia runtime/staging pendiente |
| P0-016 | Backup | P0 | **Parcialmente mitigado — dump externo DB ejecutado 2026-06-08. Storage export bucket `reports` ejecutado 2026-06-08 (17 objetos, ~3.11 MB). Backup automatico NO disponible (Free plan). Restore drill pendiente.** | Dump externo ejecutado con pg_dump/pg_dumpall v17.10 el 2026-06-08T07:45:48Z fuera del repo. Archivos en `C:\VETNEB-BACKUPS`. Evidencia sanitizada (tamanos, SHA-256) en runbook sec. 18. Storage export bucket `reports` ejecutado el 20260608-131355 fuera del repo (17 objetos, ZIP SHA-256 en runbook sec. 19). Sin secretos ni paths privados en repo. Backup automatico no disponible por Free plan. | Evidencia registrada en runbook sec. 18 y 19. Pendiente: (1) cifrado/vault dump y ZIP, (2) restore drill (P0-017). | P0 cierra completamente cuando: dump y ZIP cifrados/vault OK + restore drill ejecutado (P0-017). |
...
| 2026-06-08 | `1bbac76` | NO-GO parcial (riesgo reducido) | Dump externo DB ejecutado y evidencia sanitizada registrada (P0-016 parcialmente mitigado). Restore drill (P0-017) y Storage export siguen pendientes. Resto de P0 sin cambio. | VETNEB | — |
| 2026-06-08 | `33966ee` | NO-GO parcial (riesgo reducido) | Storage export bucket `reports` ejecutado (17 objetos, ~3.11 MB). Evidencia sanitizada en runbook sec. 19. Cifrado/vault ZIP y restore drill (P0-017) pendientes. Resto de P0 sin cambio. | VETNEB | — |
|  |  |  |  |  |  |
