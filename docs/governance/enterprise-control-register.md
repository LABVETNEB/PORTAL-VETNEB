...
`ERM-CTRL-024` Dependency Governance transiciona a `IMPLEMENTED`: los siete workflows usan
actions pinneadas a SHA y el required check `qga-workflow-security` aplica enforcement
parser-backed. El slot 16 añade la política de supply chain, la entrada Dependabot explícita
para `/frontend`, la agrupación determinista de riesgo y un job SBOM CycloneDX estructuralmente
no bloqueante, sin dependencia persistente ni action externa nueva. Verificación remota
2026-08-02 sobre PR #1630 (head `89d00f7d9c8a4430d8f8dc209f2e0252c09ad139`): los cuatro
contextos required en `SUCCESS`, `E2E Completeness` (run `30770495510`, job `91556690305`) en
`SUCCESS`, el job `generate-sbom` (job `91556690313`) en `SUCCESS` con artifact `sbom-cyclonedx`
observado (ID `8840354235`, digest `sha256:94636f161cbf015708cad8c79bd29ca40e90b615862282866a724f1e7720ecc9`),
el review thread `PRRT_kwDOR5qlsc6VrJFz` `RESOLVED`, y Dependabot security updates habilitado y
verificado por readback administrativo (`enabled=true`, `paused=false`). `ERM-DEP-001` permanece
abierto como gap operativo hasta observar el primer PR real de Dependabot contra
...
| ERM-CTRL-023 | Release and Production Readiness | Release Readiness Review | PARTIAL | 2 | P2 | Release / Ops owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md) | Release docs, smoke scripts and app-version workflow remain in place. GitHub environments `staging` and `production` are now configured and verified per [Production Readiness Environments Evidence](../release/production-readiness-environments-evidence.md) (the baseline snapshot recorded no environments at observation time): both restrict deployment to protected branches, and `production` requires reviewer `LABVETNEB` with `prevent_self_review: false`. Deployment protection baseline: PASSED. Environment secrets: NOT_RUN. Environment variables: NOT_RUN. Runtime deployment: NOT_RUN. Production readiness verifier: NOT_RUN. Restore drill: BLOCKED. Rollback drill: BLOCKED. Production readiness: PARTIAL/BLOCKED. | ERM-REL-001 | 2026-08-01 | Por release | Configure environment secrets and variables, execute a verified runtime deployment and `verify-production-readiness.mjs` against an authorized URL, and complete the Slot 13 restore and rollback drills; the already-configured `staging`/`production` environments do not need to be recreated. | Environment controls or approved equivalent are evidenced and linked to release go/no-go. |
| ERM-CTRL-024 | Dependency and Supply Chain Governance | Dependency Governance | IMPLEMENTED | 3 | NONE | Security / Dependency owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [Dependabot config](../../.github/dependabot.yml); [supply-chain policy](./supply-chain-policy.md); [SBOM RFC](../architecture/supply-chain-sbom-rfc.md); [slot 16 audit](../audit/pr-deps-supply-chain-governance-audit.md); [SBOM generator](../../scripts/supply-chain/generate-sbom.mjs); [workflow security policy](../../scripts/governance/workflow-security-policy.mjs); [workflow security validator](../../scripts/governance/workflow-security-validator.mjs); [QGA workflow](../../.github/workflows/qga-governance.yml) | The seven workflows pin external actions to lowercase 40-character SHAs and declare `contents: read`; `qga-workflow-security` enforces parser-backed action, permission and container policy as a required check. Slot 16 adds the approved supply-chain policy, an explicit `/frontend` npm Dependabot entry — configured and contract-verified — deterministic minor+patch risk grouping per ecosystem, and a `generate-sbom` job that emits a CycloneDX 1.6 artifact from `pnpm-lock.yaml` without any new dependency or external action. The job declares no `needs`, no job depends on it, and the required `validate-backend` context never observes it, so it is structurally non-blocking without `continue-on-error`. Local evidence: 21 supply-chain contract tests, workflow security validator exit 0, `pnpm validate:local` with 4,102 tests, both audits clean, and a 618-component deterministic SBOM. Remote verification (2026-08-02) on PR #1630, head `89d00f7d9c8a4430d8f8dc209f2e0252c09ad139`: the four required contexts (`validate-pr-governance`, `validate-backend`, `validate-frontend`, `qga-workflow-security`) in `SUCCESS`; `E2E Completeness` run `30770495510` / job `91556690305` in `SUCCESS`; Backend CI `pull_request` run `30770495535` in `SUCCESS` against the exact head; job `generate-sbom` (job `91556690313`) in `SUCCESS`; SBOM artifact verified — ID `8840354235`, name `sbom-cyclonedx`, 59515 bytes, digest `sha256:94636f161cbf015708cad8c79bd29ca40e90b615862282866a724f1e7720ecc9`, downloaded file SHA-256 `6e355dfbb36d9e6486ce0c3a553936cadfde0050bc9066c2b8a03caab37e2dd6`; review thread `PRRT_kwDOR5qlsc6VrJFz` (root `3696706464`, reply `3700543458`) `RESOLVED` with corrective implementation retained in the current head. Dependabot security updates: `ENABLED_VERIFIED` — pre-state `enabled=false`/`paused=false`, final state `enabled=true`/`paused=false`, verified by administrative readback (`LABVETNEB`) on 2026-08-02. Frontend coverage config: `IMPLEMENTED_VERIFIED`. SBOM workflow: `IMPLEMENTED_VERIFIED_REMOTE`. SBOM artifact: `VERIFIED`. | ERM-DEP-001 | 2026-08-02 | Mensual para deps, trimestral para policy | Monitor for the first real Dependabot canary PR against `frontend/package.json` and revalidate monthly. Preserve SHA pinning, minimum permissions and required QGA enforcement; reopen if any of the verified settings, contexts or SBOM evidence regress. | Control remains implemented while Dependabot security updates stay enabled, the four required contexts and SBOM artifact generation remain verifiable, SHA pinning/QGA enforcement remain effective, and supply-chain policy plus owners stay approved; reopens on regression of any verified item or on absence of a first `/frontend` Dependabot PR beyond a reasonable monitoring window. |
| ERM-CTRL-025 | Quality Engineering and Maintainability | Quality Engineering System | PARTIAL | 2 | P1 | QA / Tech lead | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [technical debt register](./technical-debt-register.md); [slot 08 coverage audit](../audit/pr-quality-coverage-baseline-audit.md); [slot 09 backend lint audit](../audit/pr-quality-backend-lint-baseline-audit.md) | Tests and frontend lint exist. Slot 08 adds native Node coverage without thresholds or enforcement: 81.70% lines, 78.55% branches and 78.74% functions over 4,023 tests. Slot 09 adds a reproducible backend ESLint baseline over 253 files with 0 errors and 52 warnings, zero autofix and no CI enforcement. Enabled frontend rules, mutation, complexity, duplication and dead-code gates remain pending, so the capability stays `PARTIAL`. | ERM-BE-001 (closed operationally 2026-07-30); ERM-TST-001 (closed operationally 2026-07-30); ERM-QLT-001 | 2026-07-30 | Mensual hasta baseline estable | Preserve both diagnostic commands, review drift and prioritize findings by risk without treating either baseline as enforcement; establish mutation, complexity, duplication, dead-code and pending frontend-rule baselines in separately authorized scopes. | Executable reports exist for coverage and maintainability with owners and no degraded existing gates; any future thresholds or enforcement policy is separately evidenced and approved; remaining debt entries meet their closure criteria. |
...
| 2026-07-30 | Baseline ESLint backend del slot 09 | Script y flat config raíz; 253 archivos, 0 errors y 52 warnings; contrato positivo y mutaciones negativas; [auditoría del slot 09](../audit/pr-quality-backend-lint-baseline-audit.md) | `ERM-CTRL-025` conserva `PARTIAL`; `ERM-BE-001` queda cerrado operacionalmente sin autofix ni enforcement CI; mutation, complejidad, duplicación, dead-code y reglas frontend pendientes permanecen abiertos |
| 2026-08-01 | Gobernanza de supply chain del slot 16 | [Supply-chain policy](./supply-chain-policy.md); [SBOM RFC](../architecture/supply-chain-sbom-rfc.md); Dependabot con npm raíz, npm `/frontend` y github-actions separados; job `generate-sbom` no bloqueante; [auditoría del slot 16](../audit/pr-deps-supply-chain-governance-audit.md) | `ERM-CTRL-024` conserva `PARTIAL` con evidencia local; Dependabot security updates permanece `NOT_RUN` como mutación externa; el artifact SBOM permanece `NOT_RUN` hasta observar una corrida real de CI; `ERM-DEP-001` sigue abierto; se mantienen exactamente 25 capabilities |
| 2026-08-02 | Cierre de evidencia final del slot 16 | PR #1630 (head `89d00f7d9c8a4430d8f8dc209f2e0252c09ad139`); cuatro required contexts en `SUCCESS`; `E2E Completeness` run `30770495510`/job `91556690305`; Backend CI run `30770495535`; job `generate-sbom` `91556690313` con artifact `sbom-cyclonedx` (ID `8840354235`) verificado; review thread `PRRT_kwDOR5qlsc6VrJFz` `RESOLVED`; readback administrativo de Dependabot security updates (`enabled=true`, `paused=false`); [auditoría del slot 16](../audit/pr-deps-supply-chain-governance-audit.md) | `ERM-CTRL-024` transiciona a `IMPLEMENTED` con maturity 3 y priority `NONE`; `ERM-DEP-001` permanece abierto como gap operativo hasta el primer PR real de Dependabot contra `frontend/package.json`; production readiness no cambia y sigue `PARTIAL/BLOCKED`; se mantienen exactamente 25 capabilities |
