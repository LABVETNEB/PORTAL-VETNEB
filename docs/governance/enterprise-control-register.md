...
`ERM-CTRL-013` CI/CD Pipeline Governance diverge justificadamente del snapshot: el mapa vigente distingue el required check global `validate-pr-governance` de Backend CI y Frontend CI condicionales por paths. La PR #1447 demuestra el camino negativo fallido y cerrado sin merge; las PR #1448 y #1449 demuestran caminos positivos exitosos. `ERM-CI-001` queda cerrado operativamente desde 2026-07-12 sin reescribir el gap register histórico.
...
| ERM-CTRL-013 | CI/CD and Quality Gate Governance | CI/CD Pipeline Governance | IMPLEMENTED | 3 | NONE | CI owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [CI checks runbook](../ops/CI_PR_CHECKS_RUNBOOK.md); [implementation closeout](../implementation/ci-pipeline-governance-closeout.md); [closeout audit](../audit/ci-pipeline-governance-closeout-audit.md) | The effective pipeline map distinguishes the strict required `validate-pr-governance` context from conditional Backend CI and Frontend CI jobs. PR #1447 / run `29212530876` provides the failed negative path closed without merge; PR #1448 / run `29212737354` and PR #1449 / run `29213010708` provide successful positive paths through the required gate. | ERM-CI-001 (closed operationally 2026-07-12; historical snapshot retained) | 2026-07-12 | Mensual y ante cambios de workflows, job names, path filters, branch protection o Actions permissions | Revalidate the effective check map and canary evidence after CI or protection changes; reopen on required-context drift, merge bypass or weakened triggers. | Control remains implemented while the required check is mapped and enforced, conditional CI applicability is documented, invalid PR evidence fails without merge, valid PR evidence passes, and periodic evidence is maintained. |
...
| ERM-CTRL-015 | Enterprise Repository Governance | Branch Protection Governance | IMPLEMENTED | 3 | NONE | Repository admin / Tech lead | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [review governance](../review-governance.md); [implementation closeout](../implementation/branch-protection-governance-closeout.md); [closeout audit](../audit/branch-protection-governance-closeout-audit.md) | Baseline snapshot recorded `NOT_IMPLEMENTED`. Operational evidence verifies protected PR flow on `main`, strict required check `validate-pr-governance`, administrator enforcement, linear history, conversation resolution, disabled force pushes and disabled branch deletion. PR #1446 provides the positive path; PR #1447 and workflow run `29212530876` provide a failed required-check canary closed without merge; the canary branch was deleted and its absence verified. | ERM-GOV-001 (closed operationally 2026-07-12; historical snapshot retained) | 2026-07-12 | Mensual y ante cambios CI | Revalidate the sanitized configuration and required-check context monthly and after any branch-protection, workflow, administrator or repository-plan change; reopen on drift. | Control remains implemented while protected PR flow, strict `validate-pr-governance`, administrator enforcement, linear history, conversation resolution and disabled force-push/delete settings remain effective and periodic evidence is maintained. |
