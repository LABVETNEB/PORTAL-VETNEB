...
| ERM-CTRL-011 | Enterprise Test Architecture | Enterprise Test Architecture | IMPLEMENTED | 3 | P1 | QA / Backend owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [E2E catalog](../../frontend/e2e/suites/catalog.ts); [catalog contract](../../test/architecture/e2e-suite-catalog-completeness.test.ts); [slot 06 audit](../audit/pr-e2e-ci-completeness-audit.md) | The typed catalog tracks all 72 Playwright specs exactly once. `ci` (43), `extended` (24), `evidence` (2) and `visual-linux` (3) partition `full` exactly; missing and duplicate catalog mutations fail deterministically. Slot 06 adds parser-backed workflow coverage without a parallel literal spec list. Coverage and mutation strength remain separate future quality gaps. | ERM-TST-001; GAP-TEST-1 (pending live evidence) | 2026-07-30 | Mensual y ante cambios de catálogo, runner o workflows E2E | Revalidate catalog inventory, exact partition and automatic workflow union after E2E infrastructure changes; complete the separate coverage baseline without weakening existing tests. | Control remains implemented while tracked specs are cataloged exactly once, execution cohorts partition `full`, workflow coverage derives from the catalog and negative mutations fail closed. |
...
| 2026-07-30 | Implementación local de E2E CI completeness | Workflow automático `e2e:full`; contrato parser-backed; [RFC](../audit/pr-e2e-ci-completeness-rfc.md); [auditoría del slot 06](../audit/pr-e2e-ci-completeness-audit.md) | `ERM-CTRL-011`, `ERM-CTRL-013` y `ERM-CTRL-014` conservan `IMPLEMENTED`; `GAP-TEST-1` y `PR-CI-3` permanecen `PENDING_LIVE_EVIDENCE` hasta observar 72/72 verde en la única PR; se mantienen exactamente 25 capabilities |
