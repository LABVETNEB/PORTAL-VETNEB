...
los contextos funcionales no required `validate-backend` y `validate-frontend`. PR #1601
implementó para ambos un detector liviano, un heavy condicional y un check final siempre
presente; #1602 y #1603 demostraron las rutas docs-only y backend. PR #1605 corrigió el rango de
detección de impacto a merge-base → head y añadió la evidencia stale-base ausente en esas dos
canarias. `ERM-CI-001` queda cerrado operativamente desde 2026-07-12 sin reescribir el gap
register histórico; branch protection continúa exigiendo solo los dos contextos globales.
...
`workflow-security-validator.mjs` como required check. `validate-backend` y `validate-frontend`
ya están presentes en todos los PR hacia `main`, con heavy condicional calculado desde el merge
base común hacia el head candidato y propagación fail-closed.
El cierre sigue bloqueado porque esos dos contextos no son required checks efectivos; el bloque
05 `PR-CI-REQUIRED-CHECKS` requiere autorización R3 separada.
...
| ERM-CTRL-013 | CI/CD and Quality Gate Governance | CI/CD Pipeline Governance | IMPLEMENTED | 3 | NONE | CI owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [CI checks runbook](../ops/CI_PR_CHECKS_RUNBOOK.md); [PR governance workflow](../../.github/workflows/pr-governance.yml); [QGA workflow](../../.github/workflows/qga-governance.yml); [Backend CI workflow](../../.github/workflows/backend-ci.yml); [Frontend CI workflow](../../.github/workflows/frontend-ci.yml); [implementation closeout](../implementation/ci-pipeline-governance-closeout.md); [closeout audit](../audit/ci-pipeline-governance-closeout-audit.md); [block 04 closeout](../audit/pr-ci-always-run-gates-audit.md) | Branch protection retains strict required contexts `validate-pr-governance` and `qga-workflow-security`. PR #1601 added an impact detector, conditional heavy and always-present fail-closed final context to Backend CI and Frontend CI. #1602 proves both heavies skip for docs-only while both final contexts succeed; #1603 proves Backend heavy runs while Frontend heavy skips and both final contexts succeed. PR #1605 replaced the direct base/head comparison with the validated common merge base through the candidate head, so base-only paths no longer trigger unrelated heavies; the stale-base case was verified on an outdated docs-only branch. The updated runbook distinguishes context presence, effective comparison range and heavy applicability. | ERM-CI-001 (closed operationally 2026-07-12; historical snapshot retained) | 2026-07-30 | Mensual y ante cambios de workflows, job names, impact detection, branch protection o Actions permissions | Revalidate both global required contexts, always-present functional contexts, heavy routing and canary evidence after CI or protection changes; reopen on context absence, routing drift, merge bypass or weakened triggers. | Control remains implemented while both global required checks are mapped and enforced, both functional contexts remain present in every PR, heavy applicability is documented and fail-closed, invalid governance evidence fails without merge, valid evidence passes, and periodic verification is maintained. |
| ERM-CTRL-014 | CI/CD and Quality Gate Governance | Quality Gate Architecture | PARTIAL | 2 | P2 | CI / Security owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [impact policy](../../scripts/governance/quality-gate-impact-policy.mjs); [impact validator](../../scripts/governance/quality-gate-impact-validator.mjs); [PR governance validator](../../scripts/governance/pr-governance-validator.mjs); [workflow security validator](../../scripts/governance/workflow-security-validator.mjs); [QGA workflow](../../.github/workflows/qga-governance.yml); [block 04 closeout](../audit/pr-ci-always-run-gates-audit.md) | Impact-aware gate routing and test taxonomy are executable. `qga-workflow-security` remains required. PR #1601 makes `validate-backend` and `validate-frontend` always present with conditional heavies and fail-closed final jobs. #1602 proves docs-only skip; #1603 proves Backend heavy plus Frontend skip; PR #1605 fixes impact detection to the merge-base range and adds stale-base evidence. Both contexts remain non-required, so failing functional gates do not yet protect every merge. | ERM-FE-001; ERM-CI-002 | 2026-07-30 | Mensual y ante cambios de workflows, policy, validators o branch protection | Preserve PR/QGA enforcement and block 04 routing; under separate R3 authorization, execute `PR-CI-REQUIRED-CHECKS` to make both functional contexts required without preparing it in this closeout. | Impact and workflow-security gates remain enforced, `validate-backend` and `validate-frontend` are effective required checks without blocking legitimate docs-only PRs, minimum permissions remain enforced, and canary plus branch-protection evidence are linked. |
