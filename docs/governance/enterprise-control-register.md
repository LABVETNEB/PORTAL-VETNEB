...
`ERM-CTRL-024` Dependency Governance conserva `PARTIAL`: los siete workflows usan actions
pinneadas a SHA y el required check `qga-workflow-security` aplica enforcement parser-backed.
El slot 16 añade localmente la política de supply chain, la entrada Dependabot explícita para
`/frontend`, la agrupación determinista de riesgo y un job SBOM CycloneDX estructuralmente no
bloqueante, sin dependencia persistente ni action externa nueva. Permanecen abiertas la
desactivación de Dependabot security updates, que es una mutación externa de settings todavía
`NOT_RUN`, y la ausencia de evidencia remota del artifact SBOM.
...
| ERM-CTRL-024 | Dependency and Supply Chain Governance | Dependency Governance | PARTIAL | 2 | P2 | Security / Dependency owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [Dependabot config](../../.github/dependabot.yml); [supply-chain policy](./supply-chain-policy.md); [SBOM RFC](../architecture/supply-chain-sbom-rfc.md); [slot 16 audit](../audit/pr-deps-supply-chain-governance-audit.md); [SBOM generator](../../scripts/supply-chain/generate-sbom.mjs); [workflow security policy](../../scripts/governance/workflow-security-policy.mjs); [workflow security validator](../../scripts/governance/workflow-security-validator.mjs); [QGA workflow](../../.github/workflows/qga-governance.yml) | The seven workflows pin external actions to lowercase 40-character SHAs and declare `contents: read`; `qga-workflow-security` enforces parser-backed action, permission and container policy as a required check. Slot 16 adds the approved supply-chain policy, an explicit `/frontend` npm Dependabot entry, deterministic minor+patch risk grouping per ecosystem, and a `generate-sbom` job that emits a CycloneDX 1.6 artifact from `pnpm-lock.yaml` without any new dependency or external action. The job declares no `needs`, no job depends on it, and the required `validate-backend` context never observes it, so it is structurally non-blocking without `continue-on-error`. Local evidence: 21 supply-chain contract tests, workflow security validator exit 0, `pnpm validate:local` with 4,102 tests, both audits clean, and a 618-component deterministic SBOM. Dependabot security updates: `NOT_RUN`. Frontend coverage config: `IMPLEMENTED_LOCAL`. SBOM workflow: `IMPLEMENTED_LOCAL` / `NOT_VERIFIED_REMOTE`. SBOM artifact: `NOT_RUN`. | ERM-DEP-001 | 2026-08-01 | Mensual para deps, trimestral para policy | Execute the external R2 mutation that enables Dependabot security updates and record the administrative readback; observe one real CI run producing the `sbom-cyclonedx` artifact; verify the `/frontend` entry opens updates against `frontend/package.json`. Preserve SHA pinning, minimum permissions and required QGA enforcement. | Security updates are enabled, frontend dependency coverage is verified or explicitly configured, SBOM evidence exists, SHA pinning/QGA enforcement remain effective, and supply-chain policy plus owners are approved. |
