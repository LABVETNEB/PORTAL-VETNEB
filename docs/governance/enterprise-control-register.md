...
| Evidence or approval reference | Evidencia local y configuración GitHub de solo lectura verificadas para `PR-AUDIT-ENTERPRISE-DOCS`; baseline sanitizada de secret protection verificada para `PR-SEC-REPO-SETTINGS` el 2026-07-29 |
...
`ERM-CTRL-015` Branch Protection Governance es una divergencia operativa respaldada por evidencia posterior: el baseline conserva el estado histórico `NOT_IMPLEMENTED`, mientras que este registro mantiene `IMPLEMENTED` desde 2026-07-12. La transición se sustenta en configuración administrativa sanitizada, ruta positiva mediante PR #1446, canaria negativa PR #1447 con `validate-pr-governance` fallido, cierre sin merge y registros durables enlazados. `ERM-GOV-001` queda cerrado operativamente sin reescribir su fila histórica.
...
`ERM-CTRL-013` CI/CD Pipeline Governance diverge justificadamente del snapshot: el mapa vigente
distingue los required checks globales `validate-pr-governance` y `qga-workflow-security` de
Backend CI y Frontend CI condicionales por paths. La PR #1447 demuestra el camino negativo fallido
y cerrado sin merge; las PR #1448 y #1449 demuestran caminos positivos exitosos.
`ERM-CI-001` queda cerrado operativamente desde 2026-07-12 sin reescribir el gap register
histórico; la verificación de branch protection del 2026-07-28 incorpora el segundo contexto.
...
`ERM-CTRL-014` Quality Gate Architecture conserva `PARTIAL`: la policy y el validador de impacto
existen, están integrados en `pr-governance-validator.mjs`, y `qga-workflow-security` aplica
`workflow-security-validator.mjs` como required check. El cierre sigue bloqueado porque
`validate-backend` y `validate-frontend` no son required checks efectivos.
...
sanitizada del 2026-07-29 confirma repositorio público bajo owner `User`, con secret scanning,
repository push protection, validity checks y non-provider patterns en `disabled`. La auditoría
de los seis archivos `docs/security/**` permite clasificarlos `PUBLIC_SANITIZED`. Secret scanning
básico y repository push protection quedan pendientes de autorización R2; validity checks y
non-provider patterns son `NOT_AVAILABLE` bajo titularidad y producto actuales. Esta evidencia no
cierra `ERM-SEC-001`, RLS runtime ni staging.
...
| ERM-CTRL-013 | CI/CD and Quality Gate Governance | CI/CD Pipeline Governance | IMPLEMENTED | 3 | NONE | CI owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [CI checks runbook](../ops/CI_PR_CHECKS_RUNBOOK.md); [PR governance workflow](../../.github/workflows/pr-governance.yml); [QGA workflow](../../.github/workflows/qga-governance.yml); [implementation closeout](../implementation/ci-pipeline-governance-closeout.md); [closeout audit](../audit/ci-pipeline-governance-closeout-audit.md) | Branch protection was verified on 2026-07-28 with strict required contexts `validate-pr-governance` and `qga-workflow-security`. Backend CI and Frontend CI remain conditional by paths and non-required. PR #1447 / run `29212530876` provides the failed governance path; PR #1448 / run `29212737354` and PR #1449 / run `29213010708` provide successful paths. | ERM-CI-001 (closed operationally 2026-07-12; historical snapshot retained) | 2026-07-28 | Mensual y ante cambios de workflows, job names, path filters, branch protection o Actions permissions | Revalidate both global required contexts, conditional job applicability and canary evidence after CI or protection changes; reopen on required-context drift, merge bypass or weakened triggers. | Control remains implemented while both global required checks are mapped and enforced, conditional CI applicability is documented, invalid governance evidence fails without merge, valid evidence passes, and periodic verification is maintained. |
| ERM-CTRL-014 | CI/CD and Quality Gate Governance | Quality Gate Architecture | PARTIAL | 2 | P2 | CI / Security owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [impact policy](../../scripts/governance/quality-gate-impact-policy.mjs); [impact validator](../../scripts/governance/quality-gate-impact-validator.mjs); [PR governance validator](../../scripts/governance/pr-governance-validator.mjs); [workflow security validator](../../scripts/governance/workflow-security-validator.mjs); [QGA workflow](../../.github/workflows/qga-governance.yml) | Impact-aware gate routing and test taxonomy are executable and integrated into `pr-governance-validator.mjs`. `qga-workflow-security` executes the trusted-base workflow security validator and is a required context. All workflows declare `contents: read`. Functional contexts `validate-backend` and `validate-frontend` remain conditional and non-required, so failing code gates do not yet protect every merge. | ERM-FE-001; ERM-CI-002 | 2026-07-28 | Mensual y ante cambios de workflows, policy, validators o branch protection | Preserve PR/QGA enforcement; make backend and frontend contexts always present before any separately authorized change that makes them required, with positive and negative canaries. | Impact and workflow-security gates remain enforced, `validate-backend` and `validate-frontend` are effective required checks without blocking legitimate docs-only PRs, minimum permissions remain enforced, and canary plus branch-protection evidence are linked. |
| ERM-CTRL-015 | Enterprise Repository Governance | Branch Protection Governance | IMPLEMENTED | 3 | NONE | Repository admin / Tech lead | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [CI checks runbook](../ops/CI_PR_CHECKS_RUNBOOK.md); [review governance](../review-governance.md); [implementation closeout](../implementation/branch-protection-governance-closeout.md); [closeout audit](../audit/branch-protection-governance-closeout-audit.md) | Baseline snapshot recorded `NOT_IMPLEMENTED`. Read-only verification on 2026-07-28 confirms protected PR flow on `main`, strict required contexts `validate-pr-governance` and `qga-workflow-security`, administrator enforcement, linear history, conversation resolution, disabled force pushes and disabled branch deletion. PR #1446 provides the original positive path; PR #1447 / run `29212530876` provides the failed required-check canary closed without merge. | ERM-GOV-001 (closed operationally 2026-07-12; historical snapshot retained) | 2026-07-28 | Mensual y ante cambios CI | Revalidate sanitized configuration and both required contexts after any branch-protection, workflow, administrator or repository-plan change; reopen on drift. | Control remains implemented while protected PR flow, both strict required contexts, administrator enforcement, linear history, conversation resolution and disabled force-push/delete settings remain effective and periodic evidence is maintained. |
| ERM-CTRL-016 | Security Governance and Hardening | Security Hardening Program | PARTIAL | 3 | P1 | Security / Backend owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [enterprise audit roadmap](../audit/enterprise-repository-maturity-audit-roadmap.md); [public repository exposure profile](../security/public-repository-exposure-profile.md); [PR-SEC-REPO-SETTINGS audit](../audit/pr-sec-repo-settings-audit.md) | Read-only verification on 2026-07-29 confirms a public repository owned by a `User`, with secret scanning, repository push protection, validity checks and non-provider patterns all `disabled`. The six tracked `docs/security/**` files were reviewed and classified `PUBLIC_SANITIZED`. Basic secret scanning and repository push protection are eligible but require separate R2 authorization; validity checks and non-provider patterns are `NOT_AVAILABLE` under current ownership/product. Runtime cross-tenant, RLS and staging evidence remain pending. | ERM-SEC-001; ERM-SEC-002; GAP-P0-1 | 2026-07-29 | Por release, trimestral y ante cambios de GitHub security settings, ownership o visibilidad | Review and approve the proposed public exposure profile; in a separately authorized R2 task, enable only basic secret scanning and repository push protection through an atomic fail-fast execution and verify each feature independently. Keep validity checks and non-provider patterns `NOT_AVAILABLE` until eligibility changes; continue runtime evidence and RLS work under their existing controls. | Eligible repository secret protection is enabled and independently verified; the public exposure profile is approved and `ACTIVE`; sanitization review remains effective; tenant/session/public-private controls have sanitized runtime evidence; RLS governance or compensating-control evidence remains linked without treating unavailable features as failures. |
...
| ERM-CTRL-025 | Quality Engineering and Maintainability | Quality Engineering System | PARTIAL | 2 | P1 | QA / Tech lead | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [technical debt register](./technical-debt-register.md) | Tests and frontend lint exist. The operational debt register records missing coverage and backend lint, disabled frontend rules and structural test coupling without treating documentation as closure. Mutation, complexity, duplication and dead-code gates also remain unimplemented. | ERM-BE-001; ERM-TST-001; ERM-QLT-001 | 2026-07-28 | Mensual hasta baseline estable | Use the debt register to review owners and triggers; establish coverage and backend-lint baselines incrementally, without mass reformatting or unrelated refactors. | Executable reports exist for coverage and maintainability with owners, a thresholds plan and no degraded existing gates; related debt entries meet their closure criteria. |
...
| 2026-07-29 | Baseline de secret protection y perfil de exposición pública | Lectura administrativa sanitizada; auditoría `PR-SEC-REPO-SETTINGS`; revisión de `docs/security/**` | `ERM-CTRL-016` conserva `PARTIAL`; dos features quedan pendientes de autorización R2 y dos `NOT_AVAILABLE`; `ERM-SEC-001`, RLS runtime y staging permanecen abiertos |
