...
| ERM-CTRL-013 | CI/CD and Quality Gate Governance | CI/CD Pipeline Governance | IMPLEMENTED | 3 | NONE | CI owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [CI checks runbook](../ops/CI_PR_CHECKS_RUNBOOK.md); [PR governance workflow](../../.github/workflows/pr-governance.yml); [QGA workflow](../../.github/workflows/qga-governance.yml); [Backend CI workflow](../../.github/workflows/backend-ci.yml); [Frontend CI workflow](../../.github/workflows/frontend-ci.yml); [implementation closeout](../implementation/ci-pipeline-governance-closeout.md); [closeout audit](../audit/ci-pipeline-governance-closeout-audit.md); [block 04 closeout](../audit/pr-ci-always-run-gates-audit.md); [block 05 closeout](../audit/pr-ci-required-checks-audit.md) | Branch protection retains `strict: true` and now requires four contexts: `validate-pr-governance` (app 15368), `qga-workflow-security` (app 4291335), `validate-backend` (app 15368) and `validate-frontend` (app 15368). PR #1601 added an impact detector, conditional heavy and always-present fail-closed final context to Backend CI and Frontend CI. #1602 proves both heavies skip for docs-only while both final contexts succeed; #1603 proves Backend heavy runs while Frontend heavy skips and both final contexts succeed. PR #1605 replaced the direct base/head comparison with the validated common merge base through the candidate head, so base-only paths no longer trigger unrelated heavies; the stale-base case was verified on an outdated docs-only branch. Block 05 added the effective required-check mutation, GitHub Actions hardening (`selected` allowlist, mandatory SHA pinning, `read` default token) and canaries #1616 and #1618. The updated runbook distinguishes context presence, effective comparison range, heavy applicability and Actions policy. | ERM-CI-001 (closed operationally 2026-07-12; historical snapshot retained) | 2026-07-30 | Mensual y ante cambios de workflows, job names, impact detection, branch protection o Actions permissions | Revalidate both global required contexts, always-present functional contexts, heavy routing and canary evidence after CI or protection changes; reopen on context absence, routing drift, merge bypass or weakened triggers. | Control remains implemented while both global required checks are mapped and enforced, both functional contexts remain present in every PR, heavy applicability is documented and fail-closed, invalid governance evidence fails without merge, valid evidence passes, and periodic verification is maintained. |
| ERM-CTRL-014 | CI/CD and Quality Gate Governance | Quality Gate Architecture | IMPLEMENTED | 3 | NONE | CI / Security owner | [baseline](../audit/enterprise-repository-maturity-baseline.md); [gap register](../audit/enterprise-repository-gap-register.md); [impact policy](../../scripts/governance/quality-gate-impact-policy.mjs); [impact validator](../../scripts/governance/quality-gate-impact-validator.mjs); [PR governance validator](../../scripts/governance/pr-governance-validator.mjs); [workflow security validator](../../scripts/governance/workflow-security-validator.mjs); [QGA workflow](../../.github/workflows/qga-governance.yml); [block 04 closeout](../audit/pr-ci-always-run-gates-audit.md); [block 05 closeout](../audit/pr-ci-required-checks-audit.md) | Impact-aware gate routing and test taxonomy are executable. `validate-backend` and `validate-frontend` are effective required checks, both with `app_id 15368`, alongside `validate-pr-governance` (15368) and `qga-workflow-security` (4291335) under `strict: true`. Canary #1616 proves a docs-only PR is not blocked: four required successes, both heavies skipped, final `mergeStateStatus: CLEAN`. Canary #1618 proves a failing backend gate blocks: `backend-heavy-validation` and `validate-backend` fail and the PR stays `BLOCKED` while still `MERGEABLE`. Canary #1617 is not valid negative evidence and is retained only as a test-discovery finding. GitHub Actions uses a `selected` allowlist with `github_owned_allowed: true`, `verified_allowed: false` and pattern `pnpm/action-setup@*`; `sha_pinning_required: true`; `default_workflow_permissions: read` and `can_approve_pull_request_reviews: false`. QGA keeps operating through app `4291335`. | ERM-FE-001; ERM-CI-002 (closed operationally 2026-07-30; historical snapshot retained) | 2026-07-30 | Mensual y ante cambios de workflows, policy, validators, branch protection o Actions permissions | Revalidate the four required contexts, the Actions allowlist, SHA pinning and default token permission after CI or protection changes; reopen on required-context removal, allowlist widening, pinning disablement or token permission escalation. | Control remains implemented while impact and workflow-security gates stay enforced, `validate-backend` and `validate-frontend` remain effective required checks without blocking legitimate docs-only PRs, minimum Actions permissions and SHA pinning remain enforced, and positive plus negative canary evidence stays linked. |
