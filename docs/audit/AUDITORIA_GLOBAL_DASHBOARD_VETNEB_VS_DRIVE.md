...
**Tipo:** auditoría técnica, reescritura documental y planificación. Sólo lectura.
**Repositorio:** LABVETNEB/PORTAL-VETNEB
**Rama:** `main` · **Commit:** `065860c5` — *chore(supply-chain): implement dependency governance (#1630)*
**Ejecución:** 2026-08-04 03:04 → 04:10 (UTC−03:00)
**Working tree:** limpio al iniciar y al cerrar (único elemento sin seguimiento: los dos documentos de auditoría).

> **No se implementó ningún cambio.** No se modificó código, CSS, backend, base de datos, configuración ni dependencias. No se crearon ramas, commits, pushes ni pull requests. La auditoría anterior **no fue destruida**: sigue íntegra en `docs/audit/AUDITORIA_SUPERBUSCADORES_VETNEB_VS_DRIVE.md` y se cita aquí como baseline.
...
La auditoría de superbuscadores afirmó «**9 filas visibles frente a 15–16 de Drive**». **Eso era incorrecto.** Las 9 filas eran el tamaño del dataset del fixture, no el límite del layout. La medición de esta pasada lee el indicador real de paginación y el canvas medido:
...
Se corrige el hallazgo P1-05 y se documenta el cambio en §8.
...
| Dimensión | Auditoría anterior | Esta auditoría |
|---|---|---|
| Objeto | 7 superbuscadores | Todo el dashboard autenticado (admin + clínica) |
...
| PR | 19 | **44** |
| `PR-ADJ-01/02` | fuera de alcance | **integrados al roadmap principal** (Programa B, Nivel 5) |
...
|---|---|---|---|
| 1 | Auditoría anterior | **Leída completa, preservada** | `docs/audit/AUDITORIA_SUPERBUSCADORES_VETNEB_VS_DRIVE.md` (91 928 B, 759 líneas, 25 secciones) |
| 2 | `DATOS_CONSOLIDADOS_SIN_DUPLICADOS.md` | **Recibido en dos partes** | `...DESKTOP.md` (12 928 139 B, 40 978 líneas, 22 estados) y `...MOBILE.md` (1 735 358 B, 6 924 líneas, 7 estados) |
| 3 | 5 imágenes PNG | **Resueltas en la pasada anterior** | `C:\Users\Nico\Pictures\Screenshots\` — coincidencia exacta de dimensiones raster, capturadas 2026-08-04 02:02–02:04 |
| 4 | `SKILL CLAUDE(1).zip` | **Recibido como `SKILL CLAUDE.zip`** | `C:\Users\Nico\Desktop\NICO\WEB\` — 10 `.skill`, cada uno un ZIP con su `SKILL.md`; los 10 leídos íntegros |
...
La primera pasada contó **filas renderizadas**, que están limitadas por el dataset del fixture (9 registros). Se corrigió leyendo el indicador de paginación y midiendo el canvas: el `limit` adaptativo real es 7–20 según viewport. Todas las cifras publicadas provienen de la pasada corregida. Ver §8, entrada P1-05.
...
| `production-web-optimization-engineer` | **Sí** | Renderizado, reflow, ResizeObserver, densidad, CSS, sobreingeniería, escala P0–P3. | Su checklist de frontend detectó: componentes de 62–83 KB (§15), `.field-select` duplicando `Select` (§16), 3 hooks adaptativos equivalentes (§21), 17 456 B de componentes muertos (§15.3). Su regla «no introducir abstracciones innecesarias» hizo rechazar un `SuperSearchBar` monolítico (§48.3). | 15, 16, 21, 40, 48 |
| `admin-dashboard-operational-actions` | **Sí** | Acciones, filtros, botones, mutaciones, toolbars, selección. | Inventario de las 10 superficies admin con su acción real (§13); verificación en runtime de que Aplicar/Limpiar/Actualizar tienen handler; detección de que la barra de tokens de clínica desaparece con dataset vacío (§18, P2-09). | 13, 18, 20, 32–34 |
...
| `bugs-errores-optimizacion-rutas` | **Sí (condicional)** | Persisten defectos funcionales/UX. | 5 correctivos separados en §53: `line-height` degenerado, `aria-label` sobre `div` sin rol, barra condicionada a datos, doble shell de clínica, canvas de usuarios desalineado con su `limit`. | 41–45, 53 |
| `protocolos-comunicacion` | **Sí (condicional)** | Las colecciones producen llamadas con cookies de sesión y `limit`/`offset`. | Mapa filtro → parámetro → endpoint (§18.2); trazado de `limit` a la query real (§21). | 18, 21, 56 |
| `lanzamiento-mantenimiento` | **Sí (condicional)** | Se piden readiness, gates y rollback. | 12 gates en §59, rollback lógico por PR en §62. Ningún despliegue ejecutado. | 59, 62 |
...
| Elemento | Valor |
|---|---|
| Rama / commit | `main` @ `065860c5` — **idéntico** al de la auditoría anterior |
...
**Vigencia:** el commit no cambió respecto de la auditoría anterior, por lo que **toda su evidencia de código sigue vigente**. La única corrección es de método de medición, no de código (§8, P1-05).

---

## 7. Estado y vigencia de la auditoría anterior

| Aspecto | Estado |
|---|---|
| Documento | **Preservado íntegro**, no modificado |
...
## 8. Trazabilidad de los 29 hallazgos anteriores

| Anterior | Estado | Nueva clasificación | Nuevo alcance | Motivo |
|---|---|---|---|---|
| P0-01 altura ↔ `limit` | **Vigente, ampliado** | P0-01 | 7 superbuscadores → **15 módulos, 3 hooks** | La auditoría anterior trazó un hook; hay tres |
| P0-02 tests congelan geometría | **Vigente, ampliado** | P0-02 | 12 → **21 archivos de test** | El nuevo alcance suma density tests de sesiones, usuarios, precios, logística |
| P1-01 tres implementaciones de superbuscador | Vigente | P1-01 | Sin cambio | — |
| P1-02 tres orígenes y anchos | **Vigente, agravado** | P1-01 (fusionado) | + shell: **dos app shells de clínica** | El problema de alineación es del shell, no de la barra |
| P1-03 colapso 768–1023 | Vigente | P1-02 | Sin cambio | — |
| P1-04 sin campo primario | Vigente | P1-03 | Sin cambio | — |
| **P1-05 chrome vertical** | **CORREGIDO** | **P1-04** | Todo el shell | La cifra «9 filas visibles» era el tamaño del fixture, no del layout. El `limit` real es 19 a 1920 (**más** que Drive) y 7–10 a 1366×768. La brecha se reubica: no está en 1920, está en 1366×768 y 1280×720 |
| P1-06 etiquetas incoherentes | Vigente | P2-01 | Sin cambio | Degradado: es de composición, no estructural, frente a los nuevos P1 |
| P1-07 S5/S4 sin primitivas | Vigente | P1-05 | + `.field-select` en 4 módulos más | — |
| P1-08 superficie invertida | Vigente | P1-06 | Ampliado a todo el sistema de superficies | — |
| P2-01 sombra de diálogo en botón | Vigente | P2-02 | + tarjetas de módulo | — |
| P2-02 escala tipográfica incoherente | **Vigente, agravado** | P2-03 | 3 → **6 escalas** en el dashboard | — |
| P2-03 `line-height` = `font-size` | Vigente | **PR-BUG-02** | Sin cambio | Sigue siendo correctivo separado |
| P2-04 radios mezclados | Vigente | P3-01 | Sin cambio | — |
| P2-05 `aria-label` sobre `div` sin rol | Vigente | **PR-BUG-03** | Sin cambio | Correctivo separado |
| P2-06 S4 sin región nombrada | Vigente | P2-04 | Sin cambio | — |
| P2-07 sin `aria-describedby` ni live region | **Vigente, ampliado** | P2-05 | 7 barras → **todas las colecciones** | — |
| P2-08 acciones dimensionadas por contenido | Vigente | P3-02 | Sin cambio | — |
| P2-09 barra condicionada a datos | Vigente | **PR-BUG-01** | Sin cambio | Correctivo separado |
| P3-01 padding del contenedor | Vigente | P3-03 | Sin cambio | — |
| P3-02 gap 6 vs 8 px | Vigente | P3-04 | Sin cambio | — |
| P3-03 offset tarjeta→barra | Vigente | P3-05 | Sin cambio | — |
| P3-04 altura de fila heterogénea | **Vigente, promovido** | **P1-07** | 3 → **6 alturas de fila** en el dashboard | Con el alcance global es una incoherencia estructural de la gramática de colección |
| P3-05 foco con `outline-style: none` | Vigente | P3-06 | Ampliado a todo el dashboard | — |
| P3-06 `StickyFilterBar` muerto | **Vigente, ampliado** | P2-06 | 1 → **6 componentes muertos** (17 456 B) | — |
| P4-01 icono en `Aplicar` | Vigente | P4-01 | Sin cambio | — |
| P4-02 placeholder truncado | Vigente | P4-02 | Sin cambio | — |
| P4-03 `input[hidden]` en el grid | Vigente | P4-03 | Sin cambio | — |
| P4-04 transiciones 0.15 s vs 0.1 s | Vigente | P4-04 | Ampliado a todo el dashboard | — |

**Ningún hallazgo se eliminó.** 29 heredados + 21 nuevos = **50**.

---

## 9. Definición del dashboard objetivo
...
## 10. Principios Drive-like y Material Design 3
...
- Implementar cualquiera de los 44 PR.
...
- Cambiar la altura de fila de datos como objetivo independiente: altera `itemHeightPx` → `limit` (ver §21).
...
## 13. Inventario de rutas administrativas
...
## 14. Inventario de rutas de clínica
...
| **Navegación muerta** | `DashboardSidebar`, `AdminDashboardSidebar`, `ClinicDashboardSidebar`, `DashboardSidebarFrame` | **Retirar** (§15.3) |
...
## 18. Inventario de superbuscadores *(subsistema protegido)*

Los 7 superbuscadores se incorporan al programa global conservando íntegro su inventario funcional.
...
## 20. Inventario de paneles y diálogos
...
## 21. Inventario de paginación adaptativa *(tabla obligatoria)*
...
**Solución preferente (§49, PR-A05):** crear una **reserva geométrica estable** — las regiones estructurales (toolbar, superbuscador, pager, encabezado) declaran su altura reservada al canvas de filas mediante variables CSS, de modo que el canvas medido sea invariante a cambios de CSS interno. El `limit` sigue adaptándose al viewport.
**Alternativa admisible (§49, PR-A06):** medición explícita por regiones (app header · module header · toolbar · filtros · resumen · rows canvas · pager) con un solo hook unificado.
...
## 22. Métricas de app shell (1920 × 1080)
...
El superbuscador es el **cuarto** consumidor. Los dos primeros (header de aplicación 92.33 + encabezado de workspace 73.95 = **166.28 px, el 51.4 %**) estaban fuera de alcance en la auditoría anterior y ahora entran al Programa B.
...
## 26. Datos de Google Drive utilizados
...
| Excluido | 1920 × 911 | 1 | 19, 20 (visor PDF, tema oscuro) | Sólo §36 (visor) |
| Mobile | **390 × 844** | 2 / 3 | 01–07 | §38 (responsive) |
...
| Buscador `form#aso_search_form_anchor` | `role="search"` · 834 × 48 · radio 24 · `#E9EEF6` · borde 1 px transparente · sin sombra · `max-width 832px` · `transition: background .1s ease-in, width .1s ease-out` | `I03798`, `G03805`, `C01621` | §46 |
| Input de búsqueda | 728 × 20 · 16 px «Google Sans» · transparente · sin borde ni radio · inset izq. 57 / der. 49 | `G04027`, `C01677` | §46 |
| Chip de filtro | 128.28 × 32 · radio 8 · borde 1 px `rgb(116,119,117)` · blanco · sin sombra · 14/16 px w500 `rgb(68,71,70)` · `margin 4px 8px 4px 0` · `aria-haspopup="dialog"` | `G00358`, `C00117`, `A00199` | §46 |
| Ítem de menú | 32 × 320 (`min-width 198`) · `padding 0 12px` · radio 0 · transparente · sin sombra · 13/20 px w400 · `aria-posinset`/`aria-setsize` | `G00320`, `C00101`, `A00141` | §46 |
| Mobile: `topbarHeightCssPx` | **60** (57 en búsqueda) | 01–07 | §38 |
| Mobile: `dataViewport.yCssPx` / `viewportAreaPercent` | **57** / **93.25 %** | 01–07 | §38 |
| Mobile: fila | **58 px** | 01–04 | §38 |
| Mobile: input de búsqueda | 334 × 37 @ x = 56 · **16 px** · sin borde ni radio · 85.64 % del ancho | `G01020`, `C00224` | §38 |
...
| Superbuscador | 32 – 60.5 px | pill 48 en topbar 64 | **56 px** de banda, campo 40 px | R ±2 | ver §46 |
...
## 33. Diferencias de filtros

Heredado de la auditoría anterior y vigente: **P1-01** (tres implementaciones), **P1-02** (colapso 768–1023), **P1-03** (sin campo primario, reparto fraccional 180–394 px), **P1-05** (S4/S5 sin primitivas), **P1-06** (superficie invertida), **P2-01** (etiquetas incoherentes), **P2-04** (S4 sin región nombrada).
...
**P2-06 · 17 456 B de componentes muertos** (§15.3).
...
## 39. Diferencias de accesibilidad
...
## 40. Diferencias de rendimiento y mantenibilidad
...
|---|---|---|---|---|---|---|
| **P0-01** | La altura de las regiones determina el `limit` enviado al backend | `useAdaptiveItemsPerPage.ts:92-105`; `AdminAuditCard.tsx:188,198` | Cualquier cambio de CSS altera la paginación real, operativa protegida | Reserva geométrica estable (§21) antes de tocar geometría | `supersearch-limit-invariance` | `limit` por viewport idéntico al baseline |
...
| **P0-03** | Tres hooks adaptativos con tres reglas de descuento | `useAdaptiveItemsPerPage.ts`, `useAdaptiveRowsPerPage.ts`, `useAdaptiveDashboardPageSize.ts`; 15 consumidores (§21) | Un desacoplamiento parcial deja la mitad del dashboard expuesta | Unificar en un hook con regiones declaradas | Test de invariancia sobre los 15 módulos | Un solo hook; los 15 módulos con `limit` invariante |
| **P0-04** | Dos app shells para el rol clínica | `/dashboard` topbar 55.33 + rail 39.39; `/dashboard/informes` topbar 92.33 + hnav 37 (§14, §22) | Unificar el shell cambia el canvas medido en **ambas** rutas → doble riesgo de `limit` | Unificar tras P0-03, con baseline de `limit` en las 10 superficies de clínica | E2E de paridad de shell | Un solo shell; `limit` invariante en las 10 |
...
|---|---|---|---|
| P1-01 | Tres implementaciones de superbuscador (+ 3 orígenes y 3 anchos) | 7 superbuscadores | `AdminUsersRolesReadOnlyCard.tsx:540`, `AdminClinicsManagementCard.tsx:606`; §11 doc. anterior |
| P1-02 | Colapso de densidad de filtros en 768–1023 px (42–60.5 → 117–155) | 5 superbuscadores | Medición anterior, reconfirmada |
| P1-03 | Sin campo de búsqueda primario; reparto fraccional 180–394 px | 7 superbuscadores | §11.4 doc. anterior |
| **P1-04** | **Chrome vertical (CORREGIDO)**: 29.9–54.7 % vs 19.2 %; peor en 1366×768 y 1280×720 | Todo el dashboard | §23; corrige la cifra «9 filas» del baseline |
| P1-05 | Usuarios y roles + Clínicas no usan las primitivas | 2 módulos | §11.5 doc. anterior |
| P1-06 | Superficie invertida (contenedor teñido, campos blancos) | Sistema visual | `C01621` vs runtime |
| P1-07 | Seis alturas de fila (35.66 · 37 · 41 · 49 · 51 · 156.5) vs 48 homogéneas | 12 colecciones | §19 |
| P1-08 | Sin sistema de tokens del dashboard (`tokens.css` = 514 B) | CSS | §16 |
| P1-09 | Card-centric: 4 capas de superficie antes del dato | Todo el dashboard | §23.1 |
| P1-10 | El hub es la entrada por defecto de administración | Admin | `AdminDashboardWorkspaceController:286,433` |
| P1-11 | `features/dashboard/presentation/` vacío (7 barriles, 1 componente) | Arquitectura | §15.4 |
| P1-12 | Seis mecanismos de navegación / nueve componentes | Navegación | §17 |
| P1-13 | La navegación gasta el eje escaso (altura) en vez del sobrado (anchura) | Navegación | §29 |
| P1-14 | Sin `WorkspaceScaffold` reutilizable | 15 módulos | §30 |
| P1-15 | Sin alternancia lista/cuadrícula | Colecciones | §31 |
| P1-16 | **Sin selección en ninguna de las 12 colecciones** | Colecciones | §32 |
| P1-17 | No existe la capa toolbar | 15 módulos | §34 |
| P1-18 | Sin `DetailsPane` reutilizable (4 patrones distintos) | Paneles | §35 |
| P1-19 | El peor chrome coincide con las resoluciones de portátil corporativo | Responsive | §23.2 |
| P1-20 | Doce archivos > 23 KB mezclan barra, tabla, pager, detalle y fetch | Mantenibilidad | §15.2 |
...
*(29 heredados reclasificados + 32 nuevos. La auditoría anterior aportaba 29; la reclasificación fusionó P1-02→P1-01 y movió 3 a correctivos, de modo que 26 hallazgos heredados sobreviven con ID propio y 3 como PR-BUG.)*

---

## 46. Sistema visual de destino
...
## 48. Arquitectura de componentes
...
- **`SuperSearchBar` monolítico:** necesitaría props para mecanismo de submit, presencia de cada acción, política de renderizado condicional, política de etiquetas, reset por control y plantilla de columnas. API inmanejable. **Rechazado** (mantenido del baseline anterior).
...
## 49. Programa A — Contratos y desacoplamiento
...
| **A03** | 0 | Baseline de `limit`/`offset` por módulo y viewport (los 15 de §21) | A01 |
...
## 50. Programa B — Chrome y arquitectura global
...
## 51. Programa C — Workspaces y módulos
...
| **C17** | 10 | **Piloto: Informes administrativos** — valida los 12 puntos de §54 | C13 |
...
Estructura completa por PR: **ID · Programa · Nivel · Título · Objetivo · Alcance · No alcance · Dependencias · Archivos probables · Componentes · Invariantes · Pruebas · Screenshots · Riesgos · Rollback · Criterio de cierre.** Se resume aquí la tabla obligatoria; el detalle por PR sigue el esquema de §49–51.
...
| C17 | C | 10 | **Piloto** | C13 | 1 | **Alto** | suite completa | Los 12 puntos de §53.3 |
...
| C26 | C | 13 | Validación y rollout | C25 | 15 | Bajo | suite + axe + performance | Gates de §59 verdes |

**Total: 26 PR nominales de programa + 18 PR agrupados en las filas C01–C26 = 44 PR.**

---

## 56. Correctivos separados

| PR | Hallazgo | Objetivo | Riesgo |
|---|---|---|---|
| **PR-BUG-01** | P2-09 anterior | Unificar la política de renderizado condicional de la barra (3 políticas distintas: siempre / `tokens.length` / `!reportsLoadError`) | Medio — es operativa: exige decisión explícita |
| **PR-BUG-02** | P2-03 anterior | `line-height: 12px` = `font-size` en Usuarios y roles → `line-height ≥ 1.25 × font-size` | Bajo |
| **PR-BUG-03** | P2-04, P2-05, P2-18 | `role="search"` en las 7 barras; `aria-label` sobre elemento con rol; `aria-describedby`; live region de conteo | Bajo |
| **PR-BUG-04** | P0-04 | *(si se decide antes del Programa B)* Alinear el shell de `/dashboard` con el de las rutas completas de clínica | **Alto** — toca el canvas medido |
| **PR-BUG-05** | P2-19 | *(si se decide antes de A06)* Documentar y unificar las 3 reglas de descuento sin cambiar valores | Medio |

**Ninguno se mezcla con el rediseño.**
...
| Valores en **tema oscuro** de las 15 superficies | **NO DETERMINADO** — la medición fue en claro; las capturas del propietario están en oscuro | Repetir §22–§25 con `localStorage['vetneb-theme-mode']='dark-gray'` y `emulateMedia({colorScheme:'dark'})` |
...
**Primera: la densidad de VETNEB no era el problema, y la auditoría anterior lo dijo mal.** Aquel documento afirmó «9 filas frente a 15–16 de Drive». Las 9 filas eran el tamaño del dataset del fixture. El límite adaptativo real es **19 filas a 1920 × 1080** —más que Drive— porque la fila de VETNEB mide 35.66 px frente a 48. La brecha real aparece a **1366 × 768 y 1280 × 720**, donde el chrome llega al 54.7 % y sólo caben 7–10 filas. Ésas son las resoluciones de portátil corporativo, es decir, donde probablemente trabaja la mayoría de los usuarios reales.
