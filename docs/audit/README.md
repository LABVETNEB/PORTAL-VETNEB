...
| [LIMPIEZA E2E.md](./LIMPIEZA%20E2E.md) | Auditoría global de caja blanca del subsistema E2E (baseline `2683f39a`) y programa de saneamiento **concluido**: fases E2E-GLOBAL-01…11 ejecutadas más el cierre post-programa posterior. Su Anexo B conserva el cierre final por riesgo, los accepted defer con owner y los hallazgos no bloqueantes. Se conserva como evidencia y closeout, no como backlog: el estado operativo de CI/E2E se consulta en [CI_PR_CHECKS_RUNBOOK.md](../ops/CI_PR_CHECKS_RUNBOOK.md), el catálogo y los workflows ejecutables. | `CLOSED` |
| [LIMPIEZA TEST GLOBAL.md](./LIMPIEZA%20TEST%20GLOBAL.md) | Auditoría global de caja blanca del subsistema de tests no-E2E (`test/**`, 562 specs; baseline técnico `ee8e7425` y reverificación de gobernanza `38fe1dfe`) y rector autosuficiente del programa de saneamiento `TEST-GLOBAL-01…13`. La auditoría primaria está completa y el roadmap es vigente; la implementación arranca con `TEST-GLOBAL-01A` (docs-only). Reclasifica como históricas las cifras de julio de `TDR-002` y de [pr-test-architecture-consolidation-audit.md](./pr-test-architecture-consolidation-audit.md). No reabre `LIMPIEZA E2E`. | `ACTIVE` |
