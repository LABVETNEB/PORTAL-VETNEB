...
| Reports | `reports` | `reports.clinic_id` | Lecturas y mutaciones clinic deben filtrar por `auth.clinicId`; admin debe validar recurso objetivo | `/api/reports`, `/api/reports/:reportId/*`, `/api/admin/reports/*` | `test/security-cross-tenant-idor-contract.test.ts`, `test/architecture/security/security-resource-ownership-boundaries.test.ts`, `test/reports-status-session-last-access-contract.test.ts` | Clinic A/B: reporte ajeno no enumerable; respuestas 403/404 sanitizadas | Abierto - pendiente runtime/staging |
