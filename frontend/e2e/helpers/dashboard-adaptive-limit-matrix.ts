...
const A03_USER_STATUSES = ["active", "inactive", "locked"] as const;
...
/**
 * The fixture server answers `/api/admin/users-roles` from its shallow 9-record
 * `LEGACY_USERS` pool unless `?dataset=high-volume` is present, and the runtime
 * never sends that flag (it is CAP-A1 test-only). Nine records cannot produce a
 * second page for ANY adaptive limit in the matrix, so this module needs the
 * same deep hermetic stub every other server-request consumer already has.
 */
export const A03_USERS_ROLES = Array.from(
  { length: A03_DEEP_DATASET_SIZE },
  (_, index) =>
    index % 4 === 0
      ? {
          ...
          userId: 50_000 + index,
          username: `admin_a03_${pad4(index)}`,
          role: "admin" as const,
          status: A03_USER_STATUSES[index % A03_USER_STATUSES.length],
          clinicId: null,
          clinicName: null,
          createdAt: FIXED_CREATED_AT,
          updatedAt: FIXED_UPDATED_AT,
      ...
      : {
          userType: "clinic" as const,
          userId: 60_000 + index,
          username: `usuario_clinica_a03_${pad4(index)}`,
          role: index % 2 === 0 ? ("clinic_owner" as const) : ("clinic_staff" as const),
          status: A03_USER_STATUSES[index % A03_USER_STATUSES.length],
          clinicId: 12 + (index % 20),
          clinicName: `Clinica A03 ${pad4(index)}`,
...
const STUB_USERS_ROLES: StubDefinition = {
  urlPattern: "**/api/admin/users-roles**",
  pathname: "/api/admin/users-roles",
  method: "GET",
  body: (url) => {
    const total = A03_USERS_ROLES.length;
    const { limit, offset } = readWindow(url, total, 9);
    const users = A03_USERS_ROLES.slice(offset, offset + limit);
    return {
      success: true,
      users,
      total,
      ...
      offset,
      totals: {
        adminUsers: A03_USERS_ROLES.filter((user) => user.userType === "admin").length,
        clinicUsers: A03_USERS_ROLES.filter((user) => user.userType === "clinic").length,
      },
      checkedBy: { adminUserId: 41, username: "admin_operaciones" },
    };
  },
...
export async function waitForAdaptiveConvergence(
  ...
  await page.evaluate(
    async ({ selector, scopeNth, quietFrames, frameBudget, context }) => {
      const nodes = document.querySelectorAll(selector);
      const node =
        scopeNth === undefined ? nodes.item(0) : nodes.item(scopeNth);

      if (!node) {
        const scopeLabel =
          scopeNth === undefined ? "" : ` at matched index ${scopeNth}`;

        throw new Error(
          `${context}: adaptive container "${selector}"${scopeLabel} is not in the document (resolved ${nodes.length} matching nodes)`,
        );
      }
...
/**
 * A third grammar exists: the clinics desktop pager states the window as a
 * RECORD RANGE ("1–9 de 400") and never renders a page ordinal, while its
 * mobile render uses "Pág. N / M". Both must be observable or the module's
 * desktop viewports cannot be measured at all.
 */
const PAGE_OR_RANGE_LABEL =
  'text=/^[\\s\\u00a0]*(?:(?:Página|Pág\\.)[\\s\\u00a0]*\\d+(?:[\\s\\u00a0]*(?:de|\\/)[\\s\\u00a0]*\\d+)?|\\d+[\\s\\u00a0]*[\\u2013-][\\s\\u00a0]*\\d+[\\s\\u00a0]*de[\\s\\u00a0]*\\d+)[\\s\\u00a0]*$/';

const PAGE_ORDINAL_PATTERN = /^\s*(?:Página|Pág\.)\s*(\d+)/;
const RECORD_RANGE_PATTERN = /^\s*(\d+)\s*[–-]\s*(\d+)\s*de\s*(\d+)\s*$/;

type PagerPosition =
  | { readonly kind: "page"; readonly pageNumber: number }
  | { readonly kind: "range"; readonly firstRecord: number };

/**
 * Reads the pager position in whichever grammar the runtime renders. Nothing is
 * inferred: a label matching no known grammar fails closed instead of being
 * silently treated as page 1.
 */
async function readPagerPosition(
  pageLabel: Locator,
  context: string,
): Promise<PagerPosition> {
  const text = (await pageLabel.innerText()).replace(/ /g, " ");

  const range = RECORD_RANGE_PATTERN.exec(text);
  if (range) return { kind: "range", firstRecord: Number(range[1]) };

  const ordinal = PAGE_ORDINAL_PATTERN.exec(text);
  if (ordinal) return { kind: "page", pageNumber: Number(ordinal[1]) };

  throw new Error(`${context}: pager label "${text}" matches no known grammar`);
}
...
export type RowIdentity =
  ...
  /**
   * Desktop and mobile render the SAME fixture identifier through different
   * elements (a read-only input vs. plain text). Both are read here so neither
   * render is skipped and no synthetic identity is invented for either.
   */
  | {
      readonly kind: "inputValueOrText";
...
async function readRowIdentities(
  ...
  if (identity.kind === "inputValue") {
    ...
    for (let index = 0; index < count; index += 1) {
      values.push(await rows.nth(index).locator(identity.selector).first().inputValue());
    }
    return values;
  }

  if (identity.kind === "inputValueOrText") {
    const count = await rows.count();
    const values: string[] = [];
    for (let index = 0; index < count; index += 1) {
      const row = rows.nth(index);
      const input = row.locator(identity.selector).first();
...
export type LeafTarget = {
  ...
  readonly pageLabelSelector: string;
...
/**
 * Desktop renders the pricing editor card whole; mobile splits it into an
 * "Editar" / "Catálogo" segmented module where only the catalog paginates. The
 * chip exists only in the mobile render, so its absence is the desktop case and
 * not a failure.
 */
const openMobilePricingCatalog = async (page: Page) => {
  const chip = page.locator(
    '[data-admin-mobile-config-chip="catalogo"] >> visible=true',
  );
...
const runMaintenanceDryRun = async (page: Page) => {
  // Desktop exposes the dry-run tab through ModuleTabs, mobile through the
  // segmented config chips. Selecting the wrong one leaves the schema section
  // on screen and the analyze control unreachable.
  for (const selector of [
    '[data-module-tab="dry-run"]',
    '[data-admin-mobile-config-chip="dry-run"]',
  ]) {
    const tab = page.locator(`${selector} >> visible=true`);
...
export const A03_OBSERVERS: Readonly<Record<A03ModuleId, ModuleObserver>> = Object.freeze({
    ...
    leaves: [
      {
        ...
        pageLabelSelector: `${adminScope("admin-clinics")} >> ${PAGE_OR_RANGE_LABEL}`,
  ...
  "admin-pricing": {
    ...
    stubs: [STUB_PRICING],
    rowIdentity: {
      kind: "inputValueOrText",
      selector: "input[readonly]",
      pattern: /(Estudio A03 \d{4})/,
    },
    ...
    leaves: [
      {
        ...
        prepare: openMobilePricingCatalog,
        rowSelectors: [
  ...
  "clinic-informes-summary": {
    ...
    cookies: [A03_CLINIC_SESSION_COOKIE],
    stubs: [],
    // Desktop renders "Informe #123", mobile "#123 · Paciente". The id is the
    // fixture identifier in BOTH, so neither render needs a synthetic identity.
    rowIdentity: { kind: "text", pattern: /#(\d+)/ },
...
export async function observeLeaf(
  ...
  const pageLabel = page.locator(`${leaf.pageLabelSelector} >> visible=true`).first();
  ...
  const firstPagePosition = await readPagerPosition(pageLabel, `${label}: page 1`);
  if (firstPagePosition.kind === "range") {
    expect(
      firstPagePosition.firstRecord,
      `${label}: page 1 must start at record 1`,
    ).toBe(1);
  } else {
    expect(firstPagePosition.pageNumber, `${label}: page 1`).toBe(1);
  }
...
async function observeServerRequestLeaf(
  ...
  // A server-paged module settles with ONE request of its own: it refetches
  // page 1 as soon as the measured limit replaces the pre-measurement fallback.
  // That request belongs to the ADAPTIVE SETTLE, not to the page transition, so
  // the observation window opens only once the network is idle. Nothing is
  // discarded here — the window is armed afterwards, so a transition that
  // really does issue more than one request still fails (§20.3).
  ...
  // Captured BEFORE the click: this is the settled page-1 position.
  const firstPagePosition = await readPagerPosition(pageLabel, `${label}: page 1`);
  ...
  // Grammar-agnostic proof that the pager advanced. The EXACT position is
  // asserted below against the observed offset, so this only waits for the
  // runtime's own update — it never selects a convenient value.
  await expect
    .poll(
      async () => {
        const position = await readPagerPosition(pageLabel, `${label}: page 2`);
        return position.kind === "range" ? position.firstRecord : position.pageNumber;
      },
      { timeout: 15_000, message: `${label}: second page reached` },
    )
    .toBeGreaterThan(
      firstPagePosition.kind === "range"
        ? firstPagePosition.firstRecord
        : firstPagePosition.pageNumber,
    );

  await waitForAdaptiveConvergence(page, leaf.convergenceSelector, `${label} page 2`);
  page.off("request", collect);

  // Naming WHAT each captured request asked for is what distinguishes a real
  // limit thrash (differing limit/offset) from a duplicated identical request.
  const capturedSummary = captured
    .map((request, index) => {
      const url = new URL(request.url);
      const window =
        transport === "next-server-action"
          ? request.body.replace(/\s+/g, " ").slice(0, 200)
          : `limit=${url.searchParams.get("limit")} offset=${url.searchParams.get("offset")}`;
      return `    #${index + 1} ${url.pathname} ${window}`;
    })
    .join("\n");
  ...
  // Exact pager position against the OBSERVED offset — no page ordinal is
  // assumed when the runtime states a record range instead.
  const secondPagePosition = await readPagerPosition(pageLabel, `${label}: page 2`);
  if (secondPagePosition.kind === "range") {
    expect(
      secondPagePosition.firstRecord,
      `${label}: page 2 range must start at offset + 1`,
    ).toBe(offset + 1);
  } else {
    expect(secondPagePosition.pageNumber, `${label}: page 2`).toBe(2);
  }
  ...
  return {
    ...
    provenance:
      transport === "next-server-action"
        ? `single ${wantedMethod} ${pathname} server action (next-action header present) observed between the next-page click and the second-page render`
        : `single ${wantedMethod} ${pathname} observed between the next-page click and the second-page render`,
  };
}
...
async function observeClientSliceLeaf(
  ...
  convergenceSelector: string,
  convergenceScopeNth: number | undefined,
