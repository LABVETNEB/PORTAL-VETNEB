...
export async function waitForAdaptiveConvergence(
  ...
  await page.evaluate(
    async ({ selector, quietFrames, frameBudget, context }) => {
      const node = document.querySelector(selector);
      if (!node) {
        throw new Error(`${context}: adaptive container "${selector}" is not in the document`);
      }
    ...
    { selector: containerSelector, quietFrames: 3, frameBudget: 600, context: label },
  );
}
...
const runMaintenanceDryRun = async (page: Page) => {
  const desktopTab = page.locator('[data-module-tab="dry-run"] >> visible=true');
  if ((await desktopTab.count()) > 0) {
    await desktopTab.first().click({ timeout: 15_000 });
  }
...
export const A03_OBSERVERS: Readonly<Record<A03ModuleId, ModuleObserver>> = Object.freeze({
    ...
    leaves: [
      {
        ...
        rowSelectors: [
          ...
          `${adminScope("audit-log")} div[class*="md:hidden"] > div`,
        ],
    ...
    leaves: [
      {
        ...
        pageLabelSelector: `${adminScope("admin-clinics")} >> ${PAGE_LABEL}`,
  ...
  "admin-pricing": {
    ...
    cookies: [A03_ADMIN_SESSION_COOKIE],
    stubs: [STUB_PRICING],
    rowIdentity: { kind: "inputValue", selector: "input[readonly]" },
    ...
    leaves: [
      {
        ...
        rowSelectors: [`${adminScope("admin-pricing")} [data-admin-pricing-item-form]`],
        pageLabelSelector: `${adminScope("admin-pricing")} [data-dashboard-pager-state="true"]`,
        nextSelector: `${adminScope("admin-pricing")} [data-dashboard-pager-next="true"]`,
      },
    ],
  },
  "informes-reports-list": {
  ...
  "clinic-informes-summary": {
    ...
    cookies: [A03_CLINIC_SESSION_COOKIE],
    stubs: [],
    rowIdentity: { kind: "text", pattern: /Informe #(\d+)/ },
...
export async function observeLeaf(
  ...
  const convergenceSelector =
    leaf.scopeNth === undefined
      ? leaf.convergenceSelector
      : `${leaf.convergenceSelector}:nth-of-type(${leaf.scopeNth + 1})`;
...
async function observeClientSliceLeaf(
  ...
  await expect(pageLabel, `${label}: second page reached`).toHaveText(pageLabelPattern(2));
  await waitForAdaptiveConvergence(page, convergenceSelector, `${label} page 2`);
