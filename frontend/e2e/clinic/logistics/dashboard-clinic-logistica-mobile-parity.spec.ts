...
// overflow, adaptive rows, nav structure, dialog content. Real, valuable
// Clinic-domain behavior — kept in full below — but never a parity contract,
// and it never opened an Admin session.
//
...
async function expectRowGeometryUnclipped(page: Page, label: string) {
  ...
  ).toBeLessThanOrEqual(geometry!.docClientHeight + MAX_DOCUMENT_SCROLL_DELTA_PX);
}
...
for (const viewport of MOBILE_VIEWPORTS) {
  test(`clinic Logística populated mobile parity at ${viewport.name}`, async ({
    ...
    await expect(async () => {
      ...
      await expectClinicMobileNav(page, viewport.name, "logistica");
