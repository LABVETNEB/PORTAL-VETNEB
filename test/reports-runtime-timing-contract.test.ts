...
const routeSource = readFileSync(
  resolve(process.cwd(), "server", "routes", "reports.fastify.ts"),
  "utf8",
);

test("reports request logging uses shared runtime timing helper", () => {
  assert.match(routeSource, /createRuntimeTimer/);
  assert.match(routeSource, /type RuntimeTimer/);
  assert.match(routeSource, /const REQUEST_TIMER_KEY = "__reportsRequestTimer"/);
