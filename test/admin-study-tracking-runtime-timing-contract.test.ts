...
test("admin study tracking request logging uses shared runtime timing helper", () => {
  assert.match(routeSource, /createRuntimeTimer/);
  assert.match(routeSource, /type RuntimeTimer/);
  assert.match(routeSource, /const REQUEST_TIMER_KEY = "__adminStudyTrackingRequestTimer"/);
