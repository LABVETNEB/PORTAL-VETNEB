...
const EXPECTED_WORKSPACE_SPEC_COUNT = 83;
const EXPECTED_CATALOG_SPEC_COUNT = 83;
...
function validateCatalog(
  ...
  assert.equal(currentUnion.length, 49);
...
test("affected selection fails closed for empty or shared changes", async () => {
  ...
  assert.equal(sharedSelection.specs.length, 49);
