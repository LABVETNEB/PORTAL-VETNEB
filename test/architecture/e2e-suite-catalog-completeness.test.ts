...
const EXPECTED_WORKSPACE_SPEC_COUNT = 82;
const EXPECTED_CATALOG_SPEC_COUNT = 82;
...
function validateCatalog(
  ...
  assert.equal(currentUnion.length, 48);
...
test("affected selection fails closed for empty or shared changes", async () => {
  ...
  assert.equal(sharedSelection.specs.length, 48);
