...
const EXPECTED_WORKSPACE_SPEC_COUNT = 90;
const EXPECTED_CATALOG_SPEC_COUNT = 90;
...
function validateCatalog(
  ...
  assert.equal(currentUnion.length, 56);
...
test("affected selection fails closed for empty or shared changes", async () => {
  ...
  assert.equal(sharedSelection.specs.length, 56);
