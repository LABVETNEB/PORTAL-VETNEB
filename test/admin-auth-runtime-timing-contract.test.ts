...
const routeSource = readFileSync(
  resolve(process.cwd(), "server", "routes", "admin-auth.fastify.ts"),
  "utf8",
);

test("admin auth request logging uses shared runtime timing helper", () => {
  assert.match(routeSource, /createRuntimeTimer/);
  assert.match(routeSource, /type RuntimeTimer/);
  assert.match(routeSource, /const REQUEST_TIMER_KEY = "__adminAuthRequestTimer"/);
