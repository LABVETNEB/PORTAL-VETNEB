...
function getSearchPattern(value: string) {
  const sanitizedValue = value.replace(/[%_]/g, "").trim();
...
export async function getReportsByClinicId(
  ...
  return await db
    ...
    .orderBy(desc(reports.uploadDate), desc(reports.createdAt))
    .limit(limit)
    .offset(offset);
}
...
export async function searchReports(
  ...
  if (query?.trim()) {
    const searchPattern = getSearchPattern(query.trim());
    conditions.push(
      or(
        ilike(reports.patientName, searchPattern),
        ilike(reports.fileName, searchPattern),
        ilike(reports.studyType, searchPattern),
      )!,
    );
  }
...
export async function getStudyTypes(clinicId: number): Promise<string[]> {
  const result = await db
    ...
    .from(reports)
    .where(
      and(eq(reports.clinicId, clinicId), sql`${reports.studyType} IS NOT NULL`),
    );

  return result
    .map((item) => item.studyType?.trim())
...
export async function deleteExpiredSessions(referenceDate = new Date()): Promise<number> {
  const deleted = await db
    .delete(activeSessions)
    .where(sql`${activeSessions.expiresAt} IS NOT NULL AND ${activeSessions.expiresAt} <= ${referenceDate}`)
