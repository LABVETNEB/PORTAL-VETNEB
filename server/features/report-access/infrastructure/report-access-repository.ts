...
export async function createReportAccessToken(
  input: Omit<
    ...
    "id" | "accessCount" | "lastAccessAt" | "revokedAt" | "createdAt" | "updatedAt"
  >,
) {
  const now = new Date();
  const result = await db.insert(reportAccessTokens).values({
    ...input,
    accessCount: 0,
    lastAccessAt: null,
    revokedAt: null,
...
export async function getReportAccessTokenById(id: number) {
  const result = await db.select().from(reportAccessTokens)
    .where(eq(reportAccessTokens.id, id)).limit(1);
  return result[0];
}

export async function getClinicScopedReportAccessToken(id: number, clinicId: number) {
  const result = await db.select().from(reportAccessTokens).where(and(
...
export async function listReportAccessTokens(params?: {
  ...
  const query = db.select().from(reportAccessTokens)
    .orderBy(desc(reportAccessTokens.createdAt)).limit(limit).offset(offset);
  if (filters.length === 0) {
    return query;
  }
  return db.select().from(reportAccessTokens).where(and(...filters))
    .orderBy(desc(reportAccessTokens.createdAt)).limit(limit).offset(offset);
}
...
export async function getReportAccessTokenWithReportByTokenHash(tokenHash: string) {
  const result = await db.select({ token: reportAccessTokens, report: reports })
